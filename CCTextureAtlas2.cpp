...
TextureAtlas2::~TextureAtlas2()
{
    CCLOGINFO("deallocing TextureAtlas2: %p", this);
...
ssize_t TextureAtlas2::getTotalQuads() const
{
    return _totalQuads;
}

ssize_t TextureAtlas2::getCapacity() const
{
    return _capacity;
}

Texture2D* TextureAtlas2::getTexture() const
{
    return _texture;
}

void TextureAtlas2::setTexture(Texture2D * var)
{
    CC_SAFE_RETAIN(var);
    CC_SAFE_RELEASE(_texture);
    _texture = var;
...
V3F_C4B_T2F_T2F_Quad* TextureAtlas2::getQuads()
{
...
void TextureAtlas2::setQuads(V3F_C4B_T2F_T2F_Quad* quads)
{
    _quads = quads;
}

// TextureAtlas2 - alloc & init

TextureAtlas2 * TextureAtlas2::create(const std::string& file, ssize_t capacity)
{
    TextureAtlas2 * textureAtlas = new (std::nothrow) TextureAtlas2();
...
TextureAtlas2 * TextureAtlas2::createWithTexture(Texture2D *texture, ssize_t capacity)
{
    TextureAtlas2 * textureAtlas = new (std::nothrow) TextureAtlas2();
...
bool TextureAtlas2::initWithFile(const std::string& file, ssize_t capacity)
{
...
bool TextureAtlas2::initWithTexture(Texture2D *texture, ssize_t capacity)
{
...
//    CCASSERT(texture != nullptr, "texture should not be null");
    ...
    _quads = (V3F_C4B_T2F_T2F_Quad*)malloc( _capacity * sizeof(V3F_C4B_T2F_T2F_Quad) );
    _indices = (GLushort *)malloc( _capacity * 6 * sizeof(GLushort) );
    
    if( ! ( _quads && _indices) && _capacity > 0) 
    {
        //CCLOG("cocos2d: TextureAtlas2: not enough memory");
        CC_SAFE_FREE(_quads);
        CC_SAFE_FREE(_indices);
    ...
    memset( _quads, 0, _capacity * sizeof(V3F_C4B_T2F_T2F_Quad) );
    memset( _indices, 0, _capacity * 6 * sizeof(GLushort) );
    
#if CC_ENABLE_CACHE_TEXTURE_DATA
    /** listen the event that renderer was recreated on Android/WP8 */
    _rendererRecreatedListener = EventListenerCustom::create(EVENT_RENDERER_RECREATED, CC_CALLBACK_1(TextureAtlas2::listenRendererRecreated, this));
...
void TextureAtlas2::listenRendererRecreated(EventCustom* /*event*/)
{  
    if (Configuration::getInstance()->supportsShareableVAO())
    {
        setupVBOandVAO();
...
std::string TextureAtlas2::getDescription() const
{
    return StringUtils::format("<TextureAtlas2 | totalQuads = %d>", static_cast<int>(_totalQuads));
}


void TextureAtlas2::setupIndices()
{
    if (_capacity == 0)
        return;
...
//TextureAtlas2 - VAO / VBO specific

void TextureAtlas2::setupVBOandVAO()
{
...
#define kQuadSize sizeof(_quads[0].bl)
    ...
    glVertexAttribPointer(GLProgram::VERTEX_ATTRIB_COLOR, 4, GL_UNSIGNED_BYTE, GL_TRUE, kQuadSize, (GLvoid*) offsetof( V3F_C4B_T2F_T2F, colors));
...
// TextureAtlas2 - Update, Insert, Move & Remove

void TextureAtlas2::updateQuad(V3F_C4B_T2F_T2F_Quad *quad, ssize_t index)
{
...
void TextureAtlas2::insertQuad(V3F_C4B_T2F_T2F_Quad *quad, ssize_t index)
{
...
void TextureAtlas2::insertQuads(V3F_C4B_T2F_T2F_Quad* quads, ssize_t index, ssize_t amount)
{
...
void TextureAtlas2::insertQuadFromIndex(ssize_t oldIndex, ssize_t newIndex)
{
    ...
    // texture coordinates
    V3F_C4B_T2F_T2F_Quad quadsBackup = _quads[oldIndex];
...
void TextureAtlas2::removeQuadAtIndex(ssize_t index)
{
...
void TextureAtlas2::removeQuadsAtIndex(ssize_t index, ssize_t amount)
{
...
void TextureAtlas2::removeAllQuads()
{
    _totalQuads = 0;
}

// TextureAtlas2 - Resize
bool TextureAtlas2::resizeCapacity(ssize_t newCapacity)
{
    ...
    V3F_C4B_T2F_T2F_Quad* tmpQuads = nullptr;
    ...
    if (_quads == nullptr)
    {
        tmpQuads = (V3F_C4B_T2F_T2F_Quad*)malloc(new_quads_size);
        if (tmpQuads != nullptr)
        {
    ...
    {
        tmpQuads = (V3F_C4B_T2F_T2F_Quad*)realloc(_quads, new_quads_size);
...
void TextureAtlas2::increaseTotalQuadsWith(ssize_t amount)
{
    CCASSERT(amount>=0, "amount >= 0");
    _totalQuads += amount;
}

void TextureAtlas2::moveQuadsFromIndex(ssize_t oldIndex, ssize_t amount, ssize_t newIndex)
{
    ...
    //create buffer
    size_t quadSize = sizeof(V3F_C4B_T2F_T2F_Quad);
    V3F_C4B_T2F_T2F_Quad* tempQuads = (V3F_C4B_T2F_T2F_Quad*)malloc( quadSize * amount);
...
void TextureAtlas2::moveQuadsFromIndex(ssize_t index, ssize_t newIndex)
{
...
void TextureAtlas2::fillWithEmptyQuadsFromIndex(ssize_t index, ssize_t amount)
{
    CCASSERT(index>=0 && amount>=0, "values must be >= 0");
    V3F_C4B_T2F_T2F_Quad quad;
